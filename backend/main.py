...
# Dossier généré par `npm run build` (React/Vite). N'existe que si le
# frontend a été buildé (ex: dans le Dockerfile). En local avec
# `npm run dev`, ce dossier n'existe pas encore et le mount ci-dessous est
# simplement ignoré — Vite sert alors le frontend sur son propre port.
...
# ----------------------------------------------------------------------
# Sert le frontend React déjà buildé (frontend/dist/), utile pour un
# déploiement Docker/mono-conteneur. Doit rester APRÈS les routes /api/...
# pour ne pas les masquer. Sur Vercel, ce mount n'est jamais atteint : le
# routage y est entièrement géré par vercel.json.
# ----------------------------------------------------------------------
